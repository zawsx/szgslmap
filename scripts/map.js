$(window).on('load', function() { var documentSettings = {};  var group2color = {};  var polygonSettings = [];  var polygonsLegend;  var completePoints = false;  var completePolygons = false;
  ...
  var mapData;/** Triggers the load of the spreadsheet and map creation */
   $.ajax({url:'./csv/Options.csv', type:'HEAD', error: function() {// Options.csv does not exist in the root level, so use Tabletop to fetch data from the Google sheet
       ...
       success: function() { var parse = function(s) {return Papa.parse(s[0], {header: true}).data } /*       Loading data from CSV files.       */
              $.when($.get('./csv/Options.csv'), $.get('./csv/Points.csv'),  $.get('./csv/Polylines.csv')).done(function(options, points, polylines) {
                function loadPolygonCsv(n) {     $.get('./csv/Polygons' + (n === 0 ? '' : n) + '.csv', function(data) {createPolygonSettings( parse([data]) )
                loadPolygonCsv(n+1)  }).fail(function() { onMapDataLoad( parse(options), parse(points), parse(polylines) ) }) }               // No more sheets to load, initialize the map  
          loadPolygonCsv(0) })  } });
